...
def get_life_expectancy(df):
    life_expectancies = list(df["Expectation of life e(x)"])
    return life_expectancies[0]

def make_spreadsheet(year_num, year_str):

    input_df = pd.read_excel('Input_Dataframes/Country_Data_' + year_str + '.xlsx')
    ...
    democracy_year = year_num
    if year_num == 2019:
        democracy_year = 2018
    democracy_data = democracy_all_years[democracy_all_years["Year"] == democracy_year]
    ...
    for country_code in country_codes:
        ...
        life_expectancy = get_life_expectancy(country_df)
        ...
        income_classifications.append(populate_variable(income_class_data, country_code, year_num))
        ...
        stability_estimates.append(
            populate_variable(stability_data, country_code, 'Political Stability and Absence of Violence'))
    ...
    output_df1.to_excel("Output_Dataframes/Inequality Index Values_" + year_str + "+code.xlsx")


def main():
    for year in range(2010, 2020):
        make_spreadsheet(year, str(year))
